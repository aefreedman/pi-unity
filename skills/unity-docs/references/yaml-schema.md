...
Prefer `unity_migrate_solution_docs` with `operation: plan`. Review its complete conflict set and exact `approvalHash`. Partial-v2/legacy hybrids require a complete exact-path override; normalized destination collisions cannot be overridden.

Apply only after explicit authorization of that exact current hash:

```text
unity_migrate_solution_docs operation=apply approvalHash=<exact-plan-hash> recoveryMode=backup ...
```

The package CLI mirrors this behavior for maintainers:

```bash
npx tsx scripts/migrate-unity-docs-schema.ts --solutions-root <path-to-docs/solutions>
npx tsx scripts/migrate-unity-docs-schema.ts --solutions-root <path-to-docs/solutions> --apply --approval-hash <exact-plan-hash> --recovery backup
```

Every apply creates staged hashes, byte backups, and a recovery journal. Use the recorded run directory to resume or roll back an interruption.
